...
public class Logo
{
  ...
  if (lc.timeToStop) {lc.timeToStop=false; error("", lc);}
  ...
  static String prs(Object l){return prs(l, Logo.base);}
  ...
  static boolean aValidOctalNumber(String str) {
    if(str.length()==0) return false;
    if(str.charAt(0)=='-')str = str.substring(1);
    if(str.length()==0) return false;
    for (int i=0;i<str.length();i++)
      if ("01234567".indexOf(str.charAt(i))==-1) return false;
    return true;
  }
...
class TokenStream
{
  ...
  Object readToken(LContext lc){
    ...
    try {if((Logo.base==8)&&Logo.aValidOctalNumber(s))
      return new Double(Long.parseLong(s, 8));}
    catch (NumberFormatException e) {}
...
class LogoCommandRunner implements Runnable {
  ...
  LogoCommandRunner(String s, LContext lc) {this(Logo.parse(s, lc), lc);}

  LogoCommandRunner(Object[] l, LContext lc) {
    listtorun=l;
    context=lc;
  }
