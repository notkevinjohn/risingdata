...
//    Console in = System.console();
  ...
  static void runCommand(String l){
    ...
    else if(lc.asking) {lc.answer=l; lc.asking=false;}
  ...
  static void logString(String str){
    if(lc.logname==null) return;
    try {
      PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(lc.logname, true)));
      writer.println(str);
      writer.close();
